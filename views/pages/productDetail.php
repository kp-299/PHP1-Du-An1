    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-10">
        ...
        <div>
            <div class="carousel w-full rounded-[2rem] overflow-hidden bg-white border border-slate-200 shadow-sm">
                ...
                <div id="<?= $slideId ?>" class="carousel-item relative w-full">
                    ...
                    <div class="absolute flex justify-between left-4 right-4 top-1/2 -translate-y-1/2">
                        <a href="#<?= $prevId ?>" class="btn btn-sm sm:btn-md btn-circle">❮</a>
                        <a href="#<?= $nextId ?>" class="btn btn-sm sm:btn-md btn-circle">❯</a>
                    </div>
                </div>
                <?php endforeach; ?>
            </div>
            ...
            <div class="grid grid-cols-4 gap-3 mt-4">
                ...
                <a href="#product-img-<?= $index + 1 ?>"
                    class="h-16 sm:h-20 rounded-2xl overflow-hidden bg-green-50 border border-slate-200 hover:ring-2 hover:ring-green-400 transition">
                    <?php if (!empty($image)): ?>
        ...
        <div class="bg-white border border-slate-200 rounded-[2rem] p-5 sm:p-8 h-fit">
            <div class="flex flex-wrap gap-2 mb-4">
                ...
                <?php if ($hasSalePrice): ?>
                <span class="badge bg-orange-100 text-orange-700 border-0">Đang giảm giá</span>
                <?php endif; ?>
            </div>
            ...
            <form action="index.php?area=client&controller=cart&action=add" method="POST" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="number" name="quantity" min="1" max="<?= htmlspecialchars($product['stock'] ?? 999) ?>"
                        value="1" class="input input-bordered rounded-2xl w-full">
                </div>

                <button type="submit" class="btn site-gradient-bg border-0 text-white rounded-2xl w-full">
                    Thêm vào giỏ hàng
                </button>
            </form>
    ...
    <div class="carousel carousel-center w-full space-x-4 rounded-box">
        ...
        <div class="carousel-item w-44 sm:w-56">
            <div class="bg-white border border-slate-200 rounded-3xl overflow-hidden w-full">
                ...
                <div class="p-3">
                    <a href="<?= $relatedDetailUrl ?>">
                        <h3 class="font-bold text-sm leading-5 h-10 overflow-hidden hover:text-green-600 transition">
                    ...
                    <a href="<?= $relatedDetailUrl ?>" class="btn btn-xs btn-outline rounded-xl w-full mt-3">
                        Xem chi tiết
                    </a>
                </div>
            </div>
