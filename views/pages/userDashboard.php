...
<section class="max-w-7xl mx-auto px-3 sm:px-4 py-8 sm:py-12">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
        <aside class="lg:col-span-3">
            <div class="bg-white border border-slate-200 rounded-[2rem] p-5 shadow-sm lg:sticky lg:top-24">
                <div class="flex items-center gap-4 mb-6">
                    <div
                        class="w-16 h-16 rounded-3xl site-gradient-bg flex items-center justify-center text-white text-2xl font-extrabold">
                        <?= htmlspecialchars(strtoupper(mb_substr($user['name'] ?? 'U', 0, 1))) ?>
                    </div>

                    <div class="min-w-0">
                        <h2 class="font-extrabold text-slate-950 truncate">
                ...
                <div class="space-y-2">
                    ...
                    <a href="index.php?area=client&controller=auth&action=logout"
                        class="block px-4 py-3 rounded-2xl font-bold bg-rose-50 text-rose-600 hover:bg-rose-100 transition">
                        🚪 Đăng xuất
                    </a>
                </div>
            </div>
        ...
        <div class="lg:col-span-9 space-y-6">
            <?php if ($activeTab === 'overview'): ?>
                <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
                    <div class="bg-white border border-slate-200 rounded-[2rem] p-5 shadow-sm">
                        ...
                        <p class="text-3xl font-extrabold mt-3"><?= htmlspecialchars($totalOrders) ?></p>
                    </div>

                    <div class="bg-white border border-slate-200 rounded-[2rem] p-5 shadow-sm">
                        <p class="text-slate-500">Đơn hiện tại</p>
                        <p class="text-3xl font-extrabold mt-3"><?= htmlspecialchars($totalCurrentOrders) ?></p>
                    </div>

                    <div class="bg-white border border-slate-200 rounded-[2rem] p-5 shadow-sm">
                        <p class="text-slate-500">Đơn hoàn thành</p>
                        <p class="text-3xl font-extrabold mt-3"><?= htmlspecialchars($totalCompletedOrders) ?></p>
                    </div>
                ...
                <div class="bg-white border border-slate-200 rounded-[2rem] p-5 sm:p-6 shadow-sm">
                    ...
                    <p class="text-slate-500 mt-2">
                        Đây là khu vực quản lý tài khoản. Sau này mình sẽ thêm verify email, mã xác nhận hủy đơn và thông
                        báo đơn hàng tại đây.
                    </p>

                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
                        <a href="index.php?area=client&controller=user&action=profile&tab=profile"
                            class="rounded-3xl bg-slate-50 border border-slate-200 p-5 hover:bg-slate-100 transition">
                            <div class="text-3xl mb-3">👤</div>
                            <h3 class="font-extrabold">Cập nhật thông tin</h3>
                        ...
                        <a href="index.php?area=client&controller=user&action=profile&tab=current_orders"
                            class="rounded-3xl bg-slate-50 border border-slate-200 p-5 hover:bg-slate-100 transition">
                            <div class="text-3xl mb-3">📦</div>
                            <h3 class="font-extrabold">Theo dõi đơn</h3>
                        ...
                        <a href="index.php?area=client&controller=user&action=profile&tab=orders"
                            class="rounded-3xl bg-slate-50 border border-slate-200 p-5 hover:bg-slate-100 transition">
                            <div class="text-3xl mb-3">🧾</div>
                            <h3 class="font-extrabold">Lịch sử mua hàng</h3>
                ...
                <div class="bg-white border border-slate-200 rounded-[2rem] p-5 sm:p-6 shadow-sm">
                    ...
                    <form action="index.php?area=client&controller=user&action=updateProfile" method="POST"
                        ...
                        <div>
                            ...
                            <input type="email" class="input input-bordered rounded-2xl w-full bg-slate-100"
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea name="address"
                                class="textarea textarea-bordered rounded-2xl w-full min-h-28"><?= userOld($old, $user, 'address') ?></textarea>
                        </div>
                    ...
                    <form action="index.php?area=client&controller=user&action=changePassword" method="POST"
                        ...
                        <div class="md:col-span-2">
                            ...
                            <input type="password" name="current_password"
                                class="input input-bordered rounded-2xl w-full <?= !empty($errors['current_password']) ? 'input-error' : '' ?>">
                        ...
                        <div>
                            ...
                            <input type="password" name="new_password"
                                class="input input-bordered rounded-2xl w-full <?= !empty($errors['new_password']) ? 'input-error' : '' ?>">
            ...
            <?php if ($activeTab === 'verify'): ?>
                <div class="bg-white border border-slate-200 rounded-[2rem] p-5 sm:p-6 shadow-sm">
                    ...
                    <p class="text-slate-500 mt-2">
                        Chức năng này mình để khung trước. Sau này sẽ thêm gửi mã qua email để xác thực tài khoản và xác
                        nhận hủy đơn hàng.
                    </p>
                    ...
                    <button class="btn btn-disabled rounded-2xl mt-4">
